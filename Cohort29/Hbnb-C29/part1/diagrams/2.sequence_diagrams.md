# Sequence Diagrams for API Calls

## Objective

The objective of this task is to create sequence diagrams for four important API calls in the HBnB Evolution application.

The diagrams show how the different layers of the system interact when handling a request:

- Presentation Layer
- Business Logic Layer
- Persistence Layer

The four API calls represented are:

1. User Registration
2. Place Creation
3. Review Submission
4. Fetching a List of Places

---

# 1. User Registration

## Description

This sequence diagram shows the process of registering a new user.

A client sends user information to the API. The request is passed to the facade, which coordinates the creation of the user. The business logic validates the data and sends it to the persistence layer to be stored.

## Sequence Diagram

```mermaid
sequenceDiagram
    actor Client
    participant UserAPI as UserAPI<br/>Presentation Layer
    participant Facade as HBnBFacade<br/>Business Logic Layer
    participant UserModel as User<br/>Business Model
    participant UserRepo as UserRepository<br/>Persistence Layer
    participant DB as Database

    Client->>UserAPI: POST /users/register
    UserAPI->>UserAPI: Validate request format

    UserAPI->>Facade: register_user(user_data)
    Facade->>UserModel: create User instance
    UserModel->>UserModel: validate user data

    Facade->>UserRepo: check if email exists
    UserRepo->>DB: SELECT user by email
    DB-->>UserRepo: email result
    UserRepo-->>Facade: email availability

    alt Email already exists
        Facade-->>UserAPI: return error
        UserAPI-->>Client: 409 Conflict
    else Email available
        Facade->>UserRepo: save new user
        UserRepo->>DB: INSERT user
        DB-->>UserRepo: confirm save
        UserRepo-->>Facade: user saved
        Facade-->>UserAPI: return created user
        UserAPI-->>Client: 201 Created
    end
```

## Flow Explanation

1. The client sends a registration request to the `UserAPI`.
2. The `UserAPI` validates the request format.
3. The request is sent to the `HBnBFacade`.
4. The facade creates a `User` object and applies business validation.
5. The persistence layer checks whether the email already exists.
6. If the email exists, an error response is returned.
7. If the email is available, the new user is saved.
8. The API returns a success response to the client.

---

# 2. Place Creation

## Description

This sequence diagram shows the process of creating a new place listing.

An authenticated user sends place information to the API. The facade coordinates the creation of the place, associates it with the owner, validates the data, and stores it through the persistence layer.

## Sequence Diagram

```mermaid
sequenceDiagram
    actor Client
    participant PlaceAPI as PlaceAPI<br/>Presentation Layer
    participant Facade as HBnBFacade<br/>Business Logic Layer
    participant PlaceModel as Place<br/>Business Model
    participant UserRepo as UserRepository<br/>Persistence Layer
    participant PlaceRepo as PlaceRepository<br/>Persistence Layer
    participant DB as Database

    Client->>PlaceAPI: POST /places
    PlaceAPI->>PlaceAPI: Validate request format
    PlaceAPI->>PlaceAPI: Verify authentication token

    PlaceAPI->>Facade: create_place(place_data, user_id)

    Facade->>UserRepo: find owner by user_id
    UserRepo->>DB: SELECT user by id
    DB-->>UserRepo: user result
    UserRepo-->>Facade: owner found

    Facade->>PlaceModel: create Place instance
    PlaceModel->>PlaceModel: validate price
    PlaceModel->>PlaceModel: validate latitude and longitude
    PlaceModel->>PlaceModel: assign owner

    Facade->>PlaceRepo: save new place
    PlaceRepo->>DB: INSERT place
    DB-->>PlaceRepo: confirm save
    PlaceRepo-->>Facade: place saved

    Facade-->>PlaceAPI: return created place
    PlaceAPI-->>Client: 201 Created
```

## Flow Explanation

1. The client sends a request to create a place.
2. The `PlaceAPI` validates the request and verifies authentication.
3. The request is sent to the `HBnBFacade`.
4. The facade retrieves the user who will own the place.
5. A `Place` instance is created.
6. The place data is validated, including price and location.
7. The place is saved using the `PlaceRepository`.
8. The API returns a success response.

---

# 3. Review Submission

## Description

This sequence diagram shows how a user submits a review for a place.

The API receives the review request and sends it to the facade. The facade checks that the user and place exist, creates the review, validates the rating and comment, and stores the review in the persistence layer.

## Sequence Diagram

```mermaid
sequenceDiagram
    actor Client
    participant ReviewAPI as ReviewAPI<br/>Presentation Layer
    participant Facade as HBnBFacade<br/>Business Logic Layer
    participant ReviewModel as Review<br/>Business Model
    participant UserRepo as UserRepository<br/>Persistence Layer
    participant PlaceRepo as PlaceRepository<br/>Persistence Layer
    participant ReviewRepo as ReviewRepository<br/>Persistence Layer
    participant DB as Database

    Client->>ReviewAPI: POST /reviews
    ReviewAPI->>ReviewAPI: Validate request format
    ReviewAPI->>ReviewAPI: Verify authentication token

    ReviewAPI->>Facade: create_review(review_data, user_id)

    Facade->>UserRepo: find user by user_id
    UserRepo->>DB: SELECT user by id
    DB-->>UserRepo: user result
    UserRepo-->>Facade: user found

    Facade->>PlaceRepo: find place by place_id
    PlaceRepo->>DB: SELECT place by id
    DB-->>PlaceRepo: place result
    PlaceRepo-->>Facade: place found

    Facade->>ReviewModel: create Review instance
    ReviewModel->>ReviewModel: validate rating
    ReviewModel->>ReviewModel: validate comment
    ReviewModel->>ReviewModel: associate user and place

    Facade->>ReviewRepo: save new review
    ReviewRepo->>DB: INSERT review
    DB-->>ReviewRepo: confirm save
    ReviewRepo-->>Facade: review saved

    Facade-->>ReviewAPI: return created review
    ReviewAPI-->>Client: 201 Created
```

## Flow Explanation

1. The client submits a review request.
2. The `ReviewAPI` validates the request and verifies authentication.
3. The request is sent to the `HBnBFacade`.
4. The facade retrieves the user submitting the review.
5. The facade retrieves the place being reviewed.
6. A `Review` instance is created.
7. The rating and comment are validated.
8. The review is associated with the user and place.
9. The review is saved.
10. The API returns a success response.

---

# 4. Fetching a List of Places

## Description

This sequence diagram shows how a client requests a list of places.

The API receives optional search criteria, such as price or location filters. The facade forwards the request to the persistence layer, which retrieves the matching places from the database.

## Sequence Diagram

```mermaid
sequenceDiagram
    actor Client
    participant PlaceAPI as PlaceAPI<br/>Presentation Layer
    participant Facade as HBnBFacade<br/>Business Logic Layer
    participant PlaceRepo as PlaceRepository<br/>Persistence Layer
    participant DB as Database

    Client->>PlaceAPI: GET /places?filters
    PlaceAPI->>PlaceAPI: Parse query parameters
    PlaceAPI->>PlaceAPI: Validate filters

    PlaceAPI->>Facade: get_places(filters)

    Facade->>PlaceRepo: find places by filters
    PlaceRepo->>DB: SELECT places matching filters
    DB-->>PlaceRepo: places result
    PlaceRepo-->>Facade: list of places

    Facade-->>PlaceAPI: return places
    PlaceAPI-->>Client: 200 OK
```

## Flow Explanation

1. The client sends a request to fetch places.
2. The `PlaceAPI` parses and validates the query parameters.
3. The request is sent to the `HBnBFacade`.
4. The facade asks the `PlaceRepository` to retrieve matching places.
5. The repository queries the database.
6. The database returns the matching records.
7. The list of places is returned to the API.
8. The API responds to the client with the results.

---

# Design Notes

These sequence diagrams follow the layered architecture defined in the package diagram.

The general flow is:

```text
Client
  ↓
Presentation Layer
  ↓
HBnBFacade
  ↓
Business Models
  ↓
Persistence Layer
  ↓
Database
```

The `HBnBFacade` is used as the central interface between the Presentation Layer and the Business Logic Layer. This keeps the API layer separated from the internal business models and persistence logic.

The diagrams also show how each request follows a clear sequence:

1. The API receives the request.
2. The request is validated.
3. The facade coordinates the business operation.
4. The business model applies the required logic.
5. The persistence layer stores or retrieves data.
6. A response is returned to the client.

---

# Conclusion

This task provides four sequence diagrams that describe important API workflows in the HBnB Evolution application.

The diagrams cover:

- User registration
- Place creation
- Review submission
- Fetching a list of places

Together, these diagrams help explain how the Presentation Layer, Business Logic Layer, and Persistence Layer interact to process API requests.